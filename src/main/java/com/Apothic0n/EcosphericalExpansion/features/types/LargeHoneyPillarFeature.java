...
public class LargeHoneyPillarFeature extends Feature<LargeDripstoneConfiguration> {
    public LargeHoneyPillarFeature(Codec<LargeDripstoneConfiguration> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makeHoney(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makeHoney(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makeHoney(BlockPos pRoot, boolean pPointingUp, RandomSource pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
                        ...
                        if (k > 0) {
                            ...
                            for(int i1 = 0; i1 < k && blockpos$mutableblockpos.getY() < l; ++i1) {
                                ...
                                if (DripstoneUtils.isEmptyOrWaterOrLava(pLevel, blockpos)) {
                                    flag = true;
                                    Block block = Blocks.HONEY_BLOCK;
