...
public class LargePrismarinePillarFeature extends Feature<LargeDripstoneConfiguration> {
    public LargePrismarinePillarFeature(Codec<LargeDripstoneConfiguration> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makePrismarine(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makePrismarine(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makePrismarine(BlockPos pRoot, boolean pPointingUp, RandomSource pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
