...
public class LargeHoneycombPillarFeature extends Feature<LargeDripstoneConfiguration> {
    public LargeHoneycombPillarFeature(Codec<LargeDripstoneConfiguration> pContext) {
        super(pContext);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWaterOrLava(worldgenlevel, blockpos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    LargePillar largepillarfeature$largepillar = makeHoneycomb(blockpos.atY(column$range.ceiling() - 1), false, random, k, config.stalactiteBluntness, config.heightScale);
                    LargePillar largepillarfeature$largepillar1 = makeHoneycomb(blockpos.atY(column$range.floor() + 1), true, random, k, config.stalagmiteBluntness, config.heightScale);
    ...
    private static LargePillar makeHoneycomb(BlockPos pRoot, boolean pPointingUp, RandomSource pRandom, int pRadius, FloatProvider pBluntnessBase, FloatProvider pScaleBase) {
                        ...
                        if (k > 0) {
                            ...
                            for(int i1 = 0; i1 < k && blockpos$mutableblockpos.getY() < l; ++i1) {
                                ...
                                if (DripstoneUtils.isEmptyOrWaterOrLava(pLevel, blockpos)) {
                                    flag = true;
                                    Block block = Blocks.HONEYCOMB_BLOCK;
