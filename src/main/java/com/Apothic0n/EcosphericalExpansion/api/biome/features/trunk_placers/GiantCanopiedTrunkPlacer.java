...
public class GiantCanopiedTrunkPlacer extends TrunkPlacer {
    ...
    public static final Codec<GiantCanopiedTrunkPlacer> CODEC = RecordCodecBuilder.create(instance -> GiantCanopiedTrunkPlacer.trunkPlacerParts(instance).and(instance.group(
    ...
    ).apply(instance, GiantCanopiedTrunkPlacer::new));
    ...
    public GiantCanopiedTrunkPlacer(int baseHeight, int heightRandA, int heightRandB, IntProvider amountOfBranches, IntProvider branchCount, IntProvider branchHorizontalLength, UniformInt branchStartOffsetFromTop, IntProvider branchEndHeight) {
    ...
    protected TrunkPlacerType<?> type() {
        return EcoTrunkPlacerType.GIANT_BRANCHING_TRUNK_PLACER.get();
    }
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter, RandomSource random, int freeTreeHeight, BlockPos pos, TreeConfiguration config) {
        ...
        boolean bl2;
        int k;
        GiantCanopiedTrunkPlacer.setDirtAt(level, blockSetter, random, pos.below(2), config);
    ...
    private FoliagePlacer.FoliageAttachment generateBranch(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> posSetter, RandomSource random, int freeTreeHeight, BlockPos pos, TreeConfiguration config, Function<BlockState, BlockState> propertySetter, Direction direction, int secondBranchStartOffsetFromTop, boolean bl, BlockPos.MutableBlockPos mutableBlockPos) {
        ...
        blockPos = blockPos.atY(((blockPos.getY()+5)/10)*10);
        int k = bl2 ? 2 : 1;
        for (int l = 0; l < k; ++l) {
